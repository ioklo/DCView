...
namespace DCView
{
    ...
    public class DCInsideCredential : ICredential
    {
        ...
        public DCInsideCredential()
        {
            ...
            {                
                // Task에서 워닝 어떻게
                Task<bool> bResult = DoLogin(new CancellationToken());
            }
        }
        ...
        public async Task<bool> Login(string id, string password, bool? saveLoginInfo, bool? autoLogin, CancellationToken token)
        {
            MemberID = id;
            MemberPassword = password;
            ...
            // 세팅 저장
            bool result = await DoLogin(token);
