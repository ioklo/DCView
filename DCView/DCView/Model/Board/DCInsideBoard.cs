...
namespace DCView
{
    ...
    {
        // 내부 변수
        string site, id, pcsite = "gall.dcinside.com";
        int page;
        ...
        public DCInsideBoard(string site, string id)
        {
            this.site = site;
            this.id = id;
        ...
        {
            return new Uri(string.Format("http://{0}/list.php?id={1}", pcsite, id), UriKind.Absolute);
        }

        public Uri GetArticleUri(Article article)
        {
            return new Uri(string.Format("http://{0}/list.php?id={1}&no={2}", pcsite, id, article.ID), UriKind.Absolute);
        }
        ...
        public Task GetNextArticleList(CancellationToken cts)
        {
            ...
            // 접속할 사이트
            string url = string.Format("http://{0}/list.php?id={1}&page={2}&nocache={3}", 
                site, 
                id, 
                page + 1, 
                DateTime.Now.Ticks);
        ...
        public Task WriteComment(Article article, string text, CancellationToken cts)
        {
            ...
            return webClient.UploadStringAsyncTask(new Uri(string.Format("http://{0}/_option_write.php", site), UriKind.Absolute), "POST", data);
        }       
